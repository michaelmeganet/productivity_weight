...
//                    debug_to_console("$key => $value");
        ...
        function search_output_data($outputTbl, $sid, $qid, $quono, $cid, $poid) {
            ...
            $qr = "SELECT * FROM $outputTbl WHERE sid = $sid AND qid = $qid AND quono = '$quono' AND cid = $cid AND poid $qr_ext";
            $objSQL = new SQL($qr);
            echo "search output data qr = $qr<br>\n<br>";
    ...
    <th>Start Date</th>
        ...
        try {
            foreach ($results as $datarow) {
                $count++;
                //call data from output
                $sid = $datarow['sid'];
                $qr2 = "SELECT * FROM $tbloutput WHERE sid = $sid AND NOT jobtype = 'jobtake' ORDER BY poid";
                $objSQL2 = new SQL($qr2);
                $outresults = $objSQL2->getResultRowArray();
                if (empty($outresults)) {
                    $outresults = array(
                        array(
                            'poid' => null, 'start_by' => null, 'date_start' => null, 'date_end' => null, 'machine_id' => null
                        )
                    );
                }
                foreach ($outresults as $results2) {
                    ...
                    try {
                        ...
                        . " completion_date = $completion_date | dateofcompletion = $dateofcompletion | jlfor = $jlfor | status = $status | poid = $poid "
